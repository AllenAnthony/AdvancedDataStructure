...
public:
	RedBlackTree();//构造函数
	int search_element(const T& k) const;//调用search_tree_node(k)
	int get_minmum(T& retmin)const;//调用get_minmum(root)
	int get_maxmum(T& retmax)const;//调用get_maxmum(root)
	int get_successor(const T& k, T& ret) const; //get_successor(pnode)
	int get_predecessor(const T& k, T& ret) const;//get_predecessor(pnode)
	int insert_key(const T& k); //调用rb_insert_fixup(newnode)
	int delete_key(const T& k);//利用二叉搜索树中的delete后，调用rb_delete_fixup(tnode);
	void inorder_tree_walk()const;//循环中序遍历
	RedBlackTreeNode<T>* get_root() const;//公有函数，返回root
	~RedBlackTree(); //调用make_empty(root)
private:
